...
def bot_weather():
    ...
    pibo.draw_image(openpibo.config['DATA_PATH'] + "icon/weather_bot.png")
    pibo.show_display()
...
def bot_picture():
    ...
    pibo.draw_image(openpibo.config['DATA_PATH'] + "icon/camera.png")
    pibo.show_display()
    time.sleep(1)

    pibo.start_camera()
    time.sleep(3)
    pibo.capture("/home/pi/x-openpibo-example/final/capture.png")
    pibo.stop_camera()
...
def bot_walk(msg):
    ...
    pibo.draw_image(openpibo.config['DATA_PATH'] + "icon/walk.png")
    pibo.show_display()
...
def listen():
    ...
    pibo.draw_image(openpibo.config['DATA_PATH'] + "icon/mic.png")
    pibo.show_display()

    # res = pibo.stt()['data']
    res = input('명령어를 입력하세요: ')

    LOG_PRINT('{}: {}'.format(sys._getframe(0).f_code.co_name, res))

    if "no result" not in res:
        pibo.draw_image(openpibo.config['DATA_PATH'] + "icon/check.png")
        pibo.show_display()
        decode(res)
...
if __name__ == "__main__":
    ...
    pibo.draw_image(openpibo.config['DATA_PATH'] + "icon/pibo_logo.png")
    pibo.show_display()
